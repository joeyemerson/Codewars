...
def pos_average(s):
    substrs = s.split(', ')
    matches = 0
    possible_matches = ( (len(substrs) * (len(substrs) - 1)) / 2) * len(substrs[0])
    for i in range(len(substrs) - 1):
        for j in range(i + 1, len(substrs)):
            for k in range(len(substrs[i])):
                if substrs[i][k] == substrs[j][k]: matches += 1
    return float("{0:.10f}".format((matches / possible_matches) * 100))
